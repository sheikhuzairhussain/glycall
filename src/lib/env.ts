...
const envSchema = z
  ...
  .superRefine((data) => {
    ...
    } else if (data.MODEL_PROVIDER.toLowerCase().includes("anthropic")) {
      if (!data.ANTHROPIC_API_KEY) {
        throw new Error(
          "ANTHROPIC_API_KEY is required when MODEL_PROVIDER contains 'anthropic'",
        );
      }
    } else {
      throw new Error("MODEL_PROVIDER must contain 'anthropic' or 'google'");
    }
  });
